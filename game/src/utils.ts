...
export function findGLTFObjectByObjectId(list: Array<any>, uuid: string) {
  return list.find((gltf: any) => gltf?.model?.uuid === uuid)
}

export function getContainerObjByChild(obj : any) : any  {
  if(obj?.userData?.isContainer) return obj
  else if(obj?.parent != null) return getContainerObjByChild(obj?.parent)
  else return null
}

export function loadApiImage (src: string) {
  if(!src) return ""
  return `${API_URL}/${src}`
}